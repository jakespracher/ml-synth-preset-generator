...
@click.option("--dest-path", default=constants.GENERATED_PRESETS_DIR)
@click.option("--generated-preset-name", default="generated_preset")
def main(
    n_samples, model_path, data_len, dest_path, generated_preset_name, config_file
):
    ...
    if config.cgan_labels:
        class_labels = list(
            map(
                lambda label: config.cgan_labels[
                    np.argmax(label)
                ],  # convert one-hot to int with argmax
                class_labels,
            )
        )
    ...
    for i in range(n_samples):
        ableton_analog.write_preset_from_vector(
            gen_presets[i], f"{generated_preset_name}_{i}_{class_labels[i]}", dest_path
        )
