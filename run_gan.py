...
@click.option('--generated-presets-dest',
              default=constants.GENERATED_PRESETS_DIR)
def main(num_epochs, batch_size, search, cgan, config_file, training_presets_path,
...
def run_search(dataset: np.array, dest_preset_path: str):
    search = {
        ...
        'discriminator_parameters': {
            'learning_rate': [.0001, .00005, .00001],
    ...
    search = [cattr.structure(config, datatypes.GANConfig)
              for config in generate_config_combinations(search)]
    Parallel(n_jobs=4)(delayed(run_indiv_search)(search[i], i, len(search),
                                                 dest_preset_path, dataset)
...
def generate_config_combinations(search_dict):
    return [dict(zip(search_dict.keys(), value))
            for value in itertools.product(*_flatten_search_dict(search_dict))]


def _flatten_search_dict(search_dict):
    return [generate_config_combinations(val) if isinstance(val, dict) else val
