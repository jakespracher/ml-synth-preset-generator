...
def extract_values_from_parameter(key, metadata_dict):
    ...
    if key in constants.CATEGORICAL_RANGES:
        param_values = expand_categorical_variable(
            key, metadata_dict, param_value_string
        )
        ...
        param_values = [extract_value_from_string(param_value_string, metadata_dict)]
    return param_values
...
def extract_range(param):
    ...
    if value_range:
        return (
            extract_number_from_string(value_range["Min"]["@Value"]),
...
def generate_xml_from_vector(
    vector: list, key_paths: list = None, default_dict: dict = None
) -> str:
    ...
    for key_index in range(len(key_paths)):
        ...
        target_dict = extract_target_metadata(preset_dict, key_paths[key_index][:-1])
...
def is_boolean_param(key):
    ...
    if "Toggle" in key or "Legato" in key or key in bools:
        return True
    else:
        return False
